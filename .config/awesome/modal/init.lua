...
local function show_box(s, map, name)
	...
	if settings.show_options then
		for _, mapping in ipairs(map) do
			...
			elseif mapping[1] ~= "onClose" then
				label = label .. "\n<b>" .. colorify(mapping[4] or mapping[1],beautiful.modebox_key)..
					"</b>\t" .. (mapping[3] or "???")
			end
		end
	...
	mbox.width = s.geometry.width --x + mar.left + mar.right
	mbox.height = s.geometry.height -- math.max(settings.height, y + mar.top + mar.bottom)
